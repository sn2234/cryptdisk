...
struct DiskCipherAesV4TestData
{
	UINT32					blocksCount;
	UINT32					blockSize;
	const unsigned char		diskKey[32];
	const unsigned char		tweakKey[32];
	const unsigned char		index[16];
...
static const DiskCipherAesV4TestData tests[]=
{
	{
		1,
		512,
...
void doTestDiskCipherAesV4Enc(const DiskCipherAesV4TestData& testData)
{
	...
	cipher.EncipherDataBlocks(0, testData.blocksCount, (const  PVOID)testData.plainText, &tmp[0]);

	BOOST_CHECK(equal(tmp.begin(), tmp.end(),
		checked_array_iterator<const unsigned char*>(testData.expectedCipherText,
		testData.blocksCount*testData.blockSize)));
}
...
void doTestDiskCipherAesV4EncInPlace(const DiskCipherAesV4TestData& testData)
{
	...
	cipher.EncipherDataBlocks(0, testData.blocksCount, &tmp[0]);
...
void doTestDiskCipherAesV4Dec(const DiskCipherAesV4TestData& testData)
{
	...
	cipher.DecipherDataBlocks(0, testData.blocksCount, (const  PVOID)testData.expectedCipherText, &tmp[0]);
...
void doTestDiskCipherAesV4DecInPlace(const DiskCipherAesV4TestData& testData)
{
	...
	cipher.DecipherDataBlocks(0, testData.blocksCount, &tmp[0]);
...
struct Add128TestData 
{
	unsigned char	src1[16];
	UINT64			src2;
	unsigned char	expected[16];
};

static const Add128TestData testsAdd128[] =
{
	{
		{0},
		0,
...
void doTestDiskCipherAesV4Add128(const Add128TestData& testData)
{
...
static const Inc128TestData testsInc128[] =
{
	{
		{0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
		{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}
...
void doTestDiskCipherAesV4Inc128(const Inc128TestData& testData)
{
